...
# Get user to select an installation path, and add it to PATH for system-wide access.
Import-FunctionIfNotExists -Command Install-ProgramPath -ScriptUri "Install-ProgramPath.tc.ht"

$destination = Install-ProgramPath "Whisper"
...
# If Python is installed, as about using a venv
if (-not ($condaFound) -and (Get-Command $python -ErrorAction SilentlyContinue)) {
    do {
        Write-Host -ForegroundColor Cyan "`n`nDo you want to use a VENV? (Yes is reccommended)"
        $useVenv = Read-Host "Answer (Y/N)"
        switch ($useVenv.ToLower()) {
            "y" {
            ...
            default {
                Write-Host "Invalid choice. Please choose y or n."
            }
        }
    } while ($useVenv.ToLower() -notin "y", "n")
}
